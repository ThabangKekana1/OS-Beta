...
export async function GET(
  ...
  try {
    ...
    if (!relations.nda?.pdf_storage_path) {
      return NextResponse.json({ ok: false, error: "Signed NDA not found." }, { status: 404 });
    }
    const object = await downloadPrivateObject(
      MIGRATION_CASE_DOCUMENT_BUCKET,
      relations.nda.pdf_storage_path,
    );
    if (!object) {
      return NextResponse.json({ ok: false, error: "Signed NDA not found." }, { status: 404 });
    }
    ...
    return new NextResponse(await object.arrayBuffer(), {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="foundation-1-nda-${safeReference}.pdf"`,
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Unable to download the NDA." },
      { status: 500 },
    );
  }
}
