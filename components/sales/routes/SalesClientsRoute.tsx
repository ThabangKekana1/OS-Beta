...
export function SalesClientsRoute({ agentId }: { agentId: string | null }) {
  const { leads, leadStages } = useAdminPortal();
  const router = useRouter();
  const [search, setSearch] = useState("");
  const [stageFilter, setStageFilter] = useState<AdminLeadStage | "All">("All");

  const visibleLeads = useMemo(
    () => (agentId ? leads.filter((lead) => lead.ownerId === agentId) : leads),
    [agentId, leads],
  );
  ...
  }, [search, stageFilter, visibleLeads]);
  ...
  return (
    <div className="flex w-full flex-col gap-4 lg:gap-5">
      <section className="app-surface rounded-[1.6rem] px-5 py-5 lg:px-6 lg:py-6">
        <AdminHeader
          eyebrow="Clients"
          title="My client profiles."
          description="Open any client profile to upload documentation and progress EOI workflow."
          actions={
